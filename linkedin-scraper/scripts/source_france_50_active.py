"""
source_france_50_active.py — Batch 50 prospects sans site mais ACTIFS sur Google.
Filtre strict :
  - INSEE actif (etablissement A + unite legale A + pas de cessation)
  - Google Places valide (CP Places == CP INSEE) -- pas d'homonyme
  - Telephone Google PRESENT (rejette les fiches sans tel)
  - Pas de site web (websiteUri vide)
  - Pas ferme (CLOSED_PERMANENTLY/TEMPORARILY exclus)
"""
...
# Tranches INSEE -> label lisible
TRANCHES_LABELS = {
    "":   "inconnu",  "NN": "non declare",
    "00": "0 salarie", "01": "1-2", "02": "3-5", "03": "6-9",
    "11": "10-19",     "12": "20-49",
}

# Codes categorie juridique INSEE -> forme courante
FORMES_JURIDIQUES = {
    "1000": "EI", "5485": "EURL", "5499": "SARL", "5410": "SARL",
    "5710": "SAS", "5720": "SASU", "5800": "SAS",
    "5599": "SA", "5510": "SA", "5599": "SA",
    "5202": "SNC",
}
...
def sirene_query(dept: str, ape: str, nombre: int = 50) -> list[dict]:
    ...
    for e in etabs:
        ...
        cat_jur = ul.get("categorieJuridiqueUniteLegale","") or ""
        ...
        results.append({
            ...
            "effectif_label":   TRANCHES_LABELS.get(eff, eff),
            "forme_juridique":  FORMES_JURIDIQUES.get(cat_jur, cat_jur),
        })
    return results
...
def main():
    ...
    for sector in SECTORS:
        ...
        for dept in DEPTS:
            ...
            for e in etabs:
                ...
                # Filtre strict : on exige une fiche Places valide (meme CP que INSEE)
                if not gmb_valid:
                    excluded_mismatch += 1
                    continue
                ...
                prospects.append({
                    ...
                    "effectif_tranche": e.get("effectif_tranche", ""),
                    "effectif_label":   e.get("effectif_label", ""),
                    "forme_juridique":  e.get("forme_juridique", ""),
                    "signal":           "sans_site_actif",
                    "dirigeant":        "",
                    "email":            "",
                    "statut":           "A contacter",
                })
    ...
    print(f"\nResultat : {len(prospects)} prospects sans site (actifs Google) retenus")
    print(f"Exclus (entreprises fermees Google) : {excluded_ferme}")
    print(f"Exclus (homonyme ou pas de fiche Places) : {excluded_mismatch}")
    print(f"Exclus (pas de telephone sur fiche) : {excluded_no_phone}\n")
    ...
    # Exports
    json_out = DATA_DIR / f"batch_france50_active_{TODAY}.json"
    with open(json_out, "w", encoding="utf-8") as f:
        json.dump(prospects, f, ensure_ascii=False, indent=2)

    import csv
    csv_out = DATA_DIR / f"phantombuster_france50_active_{TODAY}.csv"
    ...
    print(f"\nJSON  : {json_out}")
    print(f"CSV   : {csv_out}")
    print(f"Excel : {DATA_DIR / f'prospects_france50_active_{TODAY}.xlsx'}")
...
def make_excel(prospects: list):
    ...
    headers = ["#", "Nom commercial", "Raison sociale", "SIRET", "SIREN",
               ...
               "Date creation", "Effectif", "Forme juridique",
               "Signal", "Dirigeant", "Email", "Statut"]
    col_w = [4, 28, 28, 16, 12, 22, 8, 30, 7, 18, 22,
             14, 10, 8, 18, 12, 10, 14, 10, 22, 26, 13]
    ...
    for i, p in enumerate(prospects):
        ...
        vals = [p["num"], p["nom_commercial"], p["raison_sociale"],
                ...
                p["date_creation"], p.get("effectif_label", ""),
                p.get("forme_juridique", ""),
                p["signal"], p["dirigeant"],
                p["email"], p["statut"]]
        for col, val in enumerate(vals, 1):
            cell = ws.cell(row=r, column=col, value=val)
            cell.border = border
            cell.font = Font(name="Arial", size=9)
            cell.alignment = center if col in [1,4,5,7,9,13,14,16,17,18] else left_al
            if col == 13 and isinstance(val, float) and val >= 4.5:
                cell.fill = grn_fill
            elif col == 14 and isinstance(rating, float) and rating >= 4.5:
                cell.fill = grn_fill
            elif col == 20: cell.fill = org_fill
            elif col == 21: cell.fill = org_fill
            elif col == 22: cell.fill = ylw_fill
            else: cell.fill = base
    ...
    out = DATA_DIR / f"prospects_france50_active_{TODAY}.xlsx"
    wb.save(out)
